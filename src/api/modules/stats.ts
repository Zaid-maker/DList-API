...
export default class Stats {
  ...
  constructor(private guilds = Deps.get<Guilds>(Guilds)) {
    emitter.on("savedGuildCreate", (savedGuild) =>
      this.savedGuilds.push(savedGuild)
    );
  }
  ...
  general(id: string): GeneralStats {
    ...
    if (!savedGuild || !guild) return null;
  ...
  votes(id: string) {
    return this.savedGuilds.find((b) => b.id === id)?.votes;
  }

  recentVotes(id: string): VoteStats[] {
    const savedGuild = this.savedGuilds.find((b) => b.id === id);
    if (!savedGuild) return null;
    ...
    return Array(7)
      ...
      .map((date) => ({
        day: `${date.getDate().toString().padStart(2, "0")}/${(
          ...
          .toString()
          .padStart(2, "0")}`,
        count: savedGuild.votes.filter(
          (v) => v.at?.getDate() === date?.getDate()
        ).length,
      }))
      .reverse();
  }
  ...
  topVoters(id: string): TopVoterStats[] {
    ...
    return savedGuild.votes
      ...
      .map((id) => ({
        userId: id,
        count: savedGuild.votes.filter((v) => (v.by = id)).length,
      }));
  }
