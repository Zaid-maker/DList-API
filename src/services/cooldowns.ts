...
export default class Cooldowns {
  ...
  get(member: GuildMember, command: Command) {
    return this.cooldowns.find(
      (c) =>
        (c.userId === member.id && c.commandName === command.name) ||
        (c.commandName === command.name && c.guildId === member.guild.id)
    );
  }
  add(member: GuildMember, command: Command) {
    ...
    if (!this.get(member, command)) this.cooldowns.push(cooldown);
